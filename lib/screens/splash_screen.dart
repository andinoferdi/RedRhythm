...
class _SplashScreenState extends ConsumerState<SplashScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      print('SplashScreen: Initial auth state - isAuthenticated: ${authState.isAuthenticated}, isLoading: ${authState.isLoading}');
      
      // If user is clearly authenticated and not loading, skip splash
      if (authState.isAuthenticated && !authState.isLoading) {
        print('SplashScreen: User already authenticated, navigating to home');
        _navigateToHome();
        return;
      }
    });
    ...
    _minDisplayTimer = Timer(const Duration(seconds: 1), () {
      if (mounted) {
        print('SplashScreen: Minimum time elapsed');
        setState(() {
    ...
    _maxWaitTimer = Timer(const Duration(seconds: 5), () {
      if (mounted && !_hasNavigated) {
        print('SplashScreen: Maximum wait time exceeded, forcing navigation');
        final authState = ref.read(authControllerProvider);
  ...
  void _checkAndNavigate() {
    ...
    final authState = ref.read(authControllerProvider);
    print('SplashScreen: Checking navigation - isAuthenticated: ${authState.isAuthenticated}, isLoading: ${authState.isLoading}');
    
    // Don't navigate while loading
    if (authState.isLoading) {
      print('SplashScreen: Still loading, waiting...');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AuthState>(authControllerProvider, (previous, state) {
      print('SplashScreen: Auth state changed - was: ${previous?.isAuthenticated}/${previous?.isLoading}, now: ${state.isAuthenticated}/${state.isLoading}');
      _checkAndNavigate();
    });
